...
const d = require('debug')('express-compile:protocol-hook');
...
export function initializeRendererProcess(readOnlyMode) {
  ...
  // until we're set up
  if (readOnlyMode) {
    d(`Setting up express-compile in precompiled mode with cache dir: ${rootCacheDir}`);
 ...
 * {@link registerRequireExtension} are the top-level methods that express-compile
 * actually uses to intercept code that Express loads.
 *
...
export function initializeProtocolHook(compilerHost) {
  ...
  const electronCompileSetupCode = `if (window.require) require('express-compile/lib/protocol-hook').initializeRendererProcess(${compilerHost.readOnlyMode});`;
